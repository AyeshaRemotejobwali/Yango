...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $stmt->execute([$_SESSION['user_id'], "Your delivery has been booked! Fare: $$fare"]);
        echo "<script>alert('Delivery booked! Tracking started.'); startTracking($delivery_id);</script>";
    ...
    <script>
        function startTracking(deliveryId) {
            ...
            const interval = setInterval(() => {
                progress += 5;
                map.innerHTML = `Tracking Delivery #${deliveryId}: ${progress}% Complete`;
                if (progress >= 100) {
                    clearInterval(intervalId);
                    map.innerHTML = `Delivery #${deliveryId} Completed!`;
                }
            }, 2000);
        }
    </script>
...
<body>
    <div class="container">
        <div class="form-container">
            ...
            <div id="map">Enter details to start tracking</div>
        </div>
